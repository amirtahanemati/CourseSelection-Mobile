...
export default function ThemeToggle() {
  ...
  const lightOpacity = slideAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [1, 0],
  });
  const darkOpacity = slideAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 1],
  });

  return (
    // کانتینر نگهدارنده برای وسط‌چین کردن دقیق دکمه با آیکون‌های 28 پیکسلی
    <View className="justify-center items-center h-7">
      <TouchableOpacity
        activeOpacity={0.9}
        onPress={toggleTheme}
        style={{ width: 44, height: 28, transform: [{ scale: 0.75 }] }} // اسکیل دقیق برای کوچکتر شدن
      >
        <Animated.View
          style={[StyleSheet.absoluteFill, { opacity: lightOpacity }]}
          pointerEvents="none"
        >
          <Svg viewBox="0 0 69.667 44" width="100%" height="100%">
        ...
        >
          <Svg viewBox="0 0 69.667 44" width="100%" height="100%">
            <G transform="translate(3.5 3.5)">
              ...
              <G fill="#def8ff" transform="translate(3.585 1.325)">
                <Path
